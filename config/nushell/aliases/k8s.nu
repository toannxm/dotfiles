...
export def go2pod [
    ...
    # Always get the actual current context first (don't cache this)
    let actual_current_context = ((^kubectl config current-context) | complete | get stdout | str trim)

    let contexts = if $use_cache {
        let cached = (read-cache "contexts")
        if ($cached != null) {
            display-info "📦 Using cached contexts"
            # Update the indicator based on actual current context
            $cached | each {|ctx|
                $ctx | update indicator (if $ctx.context == $actual_current_context { "current" } else { "" })
            }
        } else {
            let fresh_data = (?ctx)
